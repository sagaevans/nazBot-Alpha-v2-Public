# ==========================================
# nazBot Sniper System [BETA v3.4 - HYBRID LIMIT & DYNAMIC DCA]
# FILE: bot_logic.py
# FUNGSI: Limit TP Adaptif, 7-Level DCA, Liquidity Guard
# ==========================================
...
_limit_order_memory: Dict[str, str] = {} # Memori pelacak status Limit TP
...
def execute_order(symbol: str, order_side: str, position_side: str, margin_to_use: float, is_dca: bool = False) -> bool:
    try:
        ...
        # Hanya tembak Market Entry, Jaring Limit akan diurus otomatis oleh _monitor_positions
...
# ========== OTAK HYBRID LIMIT & DYNAMIC RECOVERY ==========
        ...
        if _limit_order_memory.get(key) != state_key:
            try:
                ...
                if pos_side == 'LONG':
                    ...
                    _api_call(_client.futures_create_order, symbol=symbol, side='SELL', type='LIMIT', price=tp_str, quantity=qty_str, positionSide=pos_side, timeInForce='GTC')
                elif pos_side == 'SHORT':
                    tp_raw = entry_price * (1 - ((current_tp_target/100.0) / actual_lev))
                    sl_raw = entry_price * (1 + (SHORT_SL_ROE / actual_lev))
                    tp_str = f"{round(round(tp_raw / tick) * tick, price_prec):.{price_prec}f}"
                    sl_str = f"{round(round(sl_raw / tick) * tick, price_prec):.{price_prec}f}"
                    _api_call(_client.futures_create_order, symbol=symbol, side='BUY', type='LIMIT', price=tp_str, quantity=qty_str, positionSide=pos_side, timeInForce='GTC')
                    _api_call(_client.futures_create_order, symbol=symbol, side='BUY', type='STOP_MARKET', stopPrice=sl_str, closePosition=True, positionSide=pos_side, timeInForce='GTC', workingType='MARK_PRICE')
                
                _limit_order_memory[key] = state_key
                logger.info(f"🕸️ JARING LIMIT PASANG [{symbol}] Target: {mode_str} | Entry Rata-rata: {entry_price}")
            except Exception as e:
                logger.error(f"Gagal memasang Jaring Limit untuk {symbol}: {e}")
            ...
            try:
                ...
                if not first_run:
                    ...
                    for k in closed_keys:
                        ...
                        # --- SAPU BERSIH: Hapus Jaring Limit saat posisi TP/SL ---
                        try:
                            _api_call(_client.futures_cancel_all_open_orders, symbol=symbol)
                        except: pass
                ...
                if current_time - _last_heartbeat_time >= 60.0:
                    logger.info(f"👀 System OK [BETA v3.4] | VIP: {vip_count}/{MAX_VIP} | ALT: {alt_count}/{MAX_ALT}")
